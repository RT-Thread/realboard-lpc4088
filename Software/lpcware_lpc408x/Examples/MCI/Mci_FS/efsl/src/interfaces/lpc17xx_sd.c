...
//	if (Stat & STA_NODISK) return Stat;	/* No card in the socket */
	...
	if (SD_Init() && mci_read_configuration())
		Stat &= ~STA_NOINIT;
...
/**
  ...
  */
SD_BOOL mci_wait_for_ready (void)
{
    Timer2 = 50;    // 500ms
...
uint8_t SD_SendCommand (uint8_t cmd, uint32_t arg, uint8_t *buf, uint32_t len) 
{
    ...
    if (mci_wait_for_ready() == SD_FALSE) return 0x81;
...
SD_BOOL SD_ReadSector (uint32_t sect, uint8_t *buf, uint32_t cnt)
{
    ...
    if (cnt > 1) /* Read multiple block */
    {
        ...
        {            
            ...
            if (mci_wait_for_ready() && cnt==0) flag = SD_TRUE;
        }
    }
...
/**
  ...
  * @retval SD_TRUE or SD_FALSE. 
  */
SD_BOOL mci_read_configuration ()
{
    uint8_t buf[16];
