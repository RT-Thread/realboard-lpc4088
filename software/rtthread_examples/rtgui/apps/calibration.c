...
static rt_bool_t (*_cali_after)(calculate_data_t *data) = RT_NULL;
...
static void _calibration_data_post(rt_uint16_t x, rt_uint16_t y)
{
...
static rt_uint16_t _calibrate_x(rt_uint16_t adc_x, rt_uint16_t adc_y)
{
    rt_uint16_t result;
...
static rt_uint16_t _calibrate_y(rt_uint16_t adc_x, rt_uint16_t adc_y)
{
    rt_uint16_t result;
...
static struct rtgui_calibration_ops _ops = 
{
	_calibration_data_post,
	_calibrate_x,
	_calibrate_y,
};
...
/*
 * set callback function which is invoked when calibration finsh.
 */
...
void calibration_init(void *p)
{
    ...
    if (rt_device_find("touch")== RT_NULL)
    {
