...
//定义mempool块大小.
#define  mempll_block_size      256*1024
#define mempll_size             mempll_block_size*4
...
void wav(char* filename)
{
    ...
    {   
			  mempool=rt_malloc(mempll_size);
        rt_mp_init(&_mp, "wav_buf", mempool, mempll_size, mempll_block_size);
        is_inited = RT_TRUE;
    }
    ...
    if (fd >= 0)
    {
       ...
       do
       {
            ...
            //从文件读取数据
            len = read(fd, buf, mempll_block_size);
            //读取成功就把数据写入设备
            if (len > 0)
            {
