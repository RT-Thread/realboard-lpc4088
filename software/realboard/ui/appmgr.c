...
#include "xpm/app_32.xpm"
#include "xpm/task_32.xpm"
#include "xpm/setting_32.xpm"
#include "xpm/state_32.xpm"
...
void app_mgr_win_init(void)
{
   ...
   // pressed_image = rtgui_image_create_from_mem("xpm", (const rt_uint8_t*)home_xpm, sizeof(home_xpm), RT_FALSE);
    //unpressed_image = rtgui_image_create_from_mem("xpm", (const rt_uint8_t*)home_gray_xpm, sizeof(home_gray_xpm), RT_FALSE);
	  pressed_image = rtgui_image_create_from_mem("xpm", (const rt_uint8_t*)app_32_xpm, sizeof(app_32_xpm), RT_FALSE);
    unpressed_image = rtgui_image_create_from_mem("xpm", (const rt_uint8_t*)app_32_xpm, sizeof(app_32_xpm), RT_FALSE);
    ...
    rtgui_notebook_set_tab_width(notebook, 50);
    angle_y = rect.x1;

    /* create navigation */
    block = block_panel_create(angle_y + notebook->tab_h/2, &rect);
    RTGUI_WIDGET_BACKGROUND(block) = RTGUI_RGB(241, 241, 241);
    rtgui_notebook_add_image(notebook, "应用", RTGUI_WIDGET(block),
        pressed_image, unpressed_image);
    program_create(RTGUI_PANEL(block));
    angle_y += notebook->tab_h;
     
		pressed_image = rtgui_image_create_from_mem("xpm", (const rt_uint8_t*)task_32_xpm, sizeof(task_32_xpm), RT_FALSE);
    unpressed_image = rtgui_image_create_from_mem("xpm", (const rt_uint8_t*)task_32_xpm, sizeof(task_32_xpm), RT_FALSE);
    rtgui_notebook_get_client_rect(notebook, &rect);
    block = block_panel_create(angle_y + notebook->tab_h/2, &rect);
    RTGUI_WIDGET_BACKGROUND(block) = RTGUI_RGB(241, 241, 241);
    rtgui_notebook_add_image(notebook, "任务", RTGUI_WIDGET(block),
        pressed_image, unpressed_image);
    apps_list_create(RTGUI_PANEL(block));
    angle_y += notebook->tab_h;
    
		pressed_image = rtgui_image_create_from_mem("xpm", (const rt_uint8_t*)setting_32_xpm, sizeof(setting_32_xpm), RT_FALSE);
    unpressed_image = rtgui_image_create_from_mem("xpm", (const rt_uint8_t*)setting_32_xpm, sizeof(setting_32_xpm), RT_FALSE);
    block = block_panel_create(angle_y + notebook->tab_h/2, &rect);
    RTGUI_WIDGET_BACKGROUND(block) = RTGUI_RGB(241, 241, 241);
    rtgui_notebook_add_image(notebook, "设置", RTGUI_WIDGET(block), 
        pressed_image, unpressed_image);
    angle_y += notebook->tab_h;
		
			pressed_image = rtgui_image_create_from_mem("xpm", (const rt_uint8_t*)state_32_xpm, sizeof(state_32_xpm), RT_FALSE);
    unpressed_image = rtgui_image_create_from_mem("xpm", (const rt_uint8_t*)state_32_xpm, sizeof(state_32_xpm), RT_FALSE);
     block = block_panel_create(angle_y + notebook->tab_h/2, &rect);
    RTGUI_WIDGET_BACKGROUND(block) = RTGUI_RGB(241, 241, 241);
    rtgui_notebook_add_image(notebook, "状态", RTGUI_WIDGET(block), 
        pressed_image, unpressed_image);
    angle_y += notebook->tab_h;
